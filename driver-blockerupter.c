...
static void blockerupter_init_com(struct cgpu_info *blockerupter) {
    ...
    // Enable the UART
    usb_transfer_data(blockerupter, CP210X_TYPE_OUT, CP210X_REQUEST_IFC_ENABLE, CP210X_VALUE_UART_ENABLE, interface, NULL, 0, C_ENABLE_UART);
    if (blockerupter->usbinfo.nodev)
        return;

    // Set data control
    usb_transfer_data(blockerupter, CP210X_TYPE_OUT, CP210X_REQUEST_DATA, CP210X_VALUE_DATA, interface, NULL, 0, C_SETDATA);

    if (blockerupter->usbinfo.nodev)
        return;

    // Set the baud
    uint32_t baudrate = BET_BAUD;
    usb_transfer_data(blockerupter, CP210X_TYPE_OUT, CP210X_REQUEST_BAUD, 0, interface, &baudrate, sizeof (baudrate), C_SETBAUD);
...
static void blockerupter_setclock(struct cgpu_info *blockerupter,uint8_t clock) {
    ...
    if(!err) {
        applog(LOG_DEBUG,"%s%d: Set Clock to %d MHz",blockerupter->drv->name, blockerupter->device_id,(clock+1)*10/2);
    }
}
...
static void blockerupter_init(struct cgpu_info *blockerupter) {
    ...
    info = blockerupter->device_data;
    // Set Clock
    if(!opt_bet_clk) opt_bet_clk = BET_CLOCK_DEFAULT;
...
static struct cgpu_info *blockerupter_detect_one(struct libusb_device *dev, struct usb_find_devices *found) {
    ...
    for (i = 0; i < BET_MAXBOARDS; i++) {
        ...
        if (answer == A_WAL) {
            applog(LOG_DEBUG, "BlockEruper found Board: %d", i);
            info->boards[i] = 1;
            info->found++;
        } else {
            applog(LOG_DEBUG, "BlockErupter missing board: %d, received %02x", i, answer);
        }
    }
    ...
    if (!info->found) {
        ...
        applog(LOG_DEBUG, "Add BlockErupter with %d/%d Boards", info->found, BET_MAXBOARDS);
...
static void blockerupter_sendjob(struct cgpu_info *blockerupter, int board) {
    ...
    if (err || answer != A_GET) {
        ...
        applog(LOG_DEBUG, "%s%d: Sent work %d to board %d", blockerupter->drv->name, blockerupter->device_id, info->work_idx, board);
    }
...
static uint64_t blockerupter_checknonce(struct cgpu_info *blockerupter, struct blockerupter_response *resp, int board) {
    ...
    applog(LOG_DEBUG, "%s%d: Nonce %08x from board %d, asic %d for work %d", blockerupter->drv->name, blockerupter->device_id, *(uint32_t *) resp->nonce, board, resp->chip, resp->work_idx);
    ...
    for (i = 0; i < BET_NONCE_FIX; i++) {
        ...
        if(test) {
            applog(LOG_DEBUG, "%s%d: Nonce Fix Pass @%d", blockerupter->drv->name, blockerupter->device_id, i);
...
static int64_t blockerupter_scanhash(struct thr_info *thr) {
    ...
    if (unlikely(blockerupter->usbinfo.nodev)) {
        applog(LOG_ERR, "%s%d: Device disappeared, shutting down thread", blockerupter->drv->name, blockerupter->device_id);
        return -1;
    }
    
    for (i = 0; i < BET_MAXBOARDS; i++) {
        if (!info->boards[i]) continue;
        ask = C_ASK | (uint8_t)i;
        ...
        case A_NO:
            break;
        default:
            applog(LOG_ERR, "%s%d: Unexpected value %02x received", blockerupter->drv->name, blockerupter->device_id, answer);
            break;
        }
    }
